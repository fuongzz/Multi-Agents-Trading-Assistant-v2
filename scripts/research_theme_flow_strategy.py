"""Research VN100-wide thematic flow strategies against the MVP shared pool."""
...
THEME_CONFIG = CONFIG_DIR / "theme_flow_hypotheses.json"
OIL_CONFIG = CONFIG_DIR / "oil_gas_rotation_hypotheses.json"
OUT_ROOT = ROOT / "backtest_results" / "theme_flow_research"


def _load_hypothesis_map() -> dict[str, Hypothesis]:
    out: dict[str, Hypothesis] = {}
    for config in [Path(DEFAULT_CONFIG), GLOBAL_CONFIG, SECTOR_CONFIG, OIL_CONFIG, THEME_CONFIG]:
...
def _export_recent_passes(
    *,
    ...
    recent = features[(features["date"] >= start_ts) & (features["date"] <= end_ts)].copy()
    rows: list[dict[str, Any]] = []
    for date, daily in recent.groupby("date", sort=True):
        latest = daily.reset_index(drop=True)
        for hyp in hypotheses:
            ranked = rank_candidates(latest[evaluate_filters(latest, hyp)], hyp).head(20)
                ...
                rows.append(
                    {
                        ...
                        "sl1_state": item.get("sl1_state"),
                        "sl1_rs_rank_20d": item.get("sl1_rs_rank_20d"),
                    }
                )
    pd.DataFrame(rows).to_csv(out_dir / "recent_theme_flow_passes.csv", index=False)


def _export_strategy_breakdown(out_dir: Path, case_name: str) -> None:
    path = out_dir / f"{case_name}_trades.csv"
    if not path.exists():
        return
    ...
    breakdown.to_csv(out_dir / f"{case_name}_strategy_breakdown.csv", index=False)


def main() -> None:
    parser = argparse.ArgumentParser(description="Backtest VN100-wide thematic flow strategies")
    parser.add_argument("--universe", default="vn100")
    parser.add_argument("--start", default="2025-01-01")
    parser.add_argument("--end", default="2026-05-14")
    parser.add_argument("--label", default="theme_flow_2025now")
    ...
    print(f"[theme-flow] universe={args.universe} symbols={len(clean_symbols)}")
    print("[theme-flow] building features...")
    ...
    theme_quality_strategies = [
        "theme_flow_leader_quality_v2",
        "theme_flow_shock_reclaim_v2",
        "theme_flow_breakout_quality_v2",
    ]
    cases = [
        {"name": "THEME_FLOW_ONLY_p3", "strategies": theme_strategies, "max_positions": 3},
        {"name": "THEME_FLOW_ONLY_p5", "strategies": theme_strategies, "max_positions": 5},
        {"name": "THEME_FLOW_QUALITY_ONLY_p3", "strategies": theme_quality_strategies, "max_positions": 3},
        {"name": "THEME_FLOW_QUALITY_ONLY_p5", "strategies": theme_quality_strategies, "max_positions": 5},
        {"name": "THEME_FLOW_SHOCK_ONLY_p2", "strategies": ["theme_flow_shock_reclaim_v2"], "max_positions": 2},
        {"name": "THEME_FLOW_SHOCK_ONLY_p3", "strategies": ["theme_flow_shock_reclaim_v2"], "max_positions": 3},
        {"name": "MVP_BASELINE_p5", "strategies": mvp_strategies, "max_positions": 5},
        {"name": "MVP_PLUS_THEME_SHARED_p5", "strategies": mvp_strategies + theme_strategies, "max_positions": 5},
        {"name": "MVP_PLUS_THEME_SHARED_p6", "strategies": mvp_strategies + theme_strategies, "max_positions": 6},
        {"name": "MVP_PLUS_THEME_QUALITY_SHARED_p5", "strategies": mvp_strategies + theme_quality_strategies, "max_positions": 5},
        {"name": "MVP_PLUS_THEME_QUALITY_SHARED_p6", "strategies": mvp_strategies + theme_quality_strategies, "max_positions": 6},
        {"name": "MVP_PLUS_THEME_SHOCK_SHARED_p5", "strategies": mvp_strategies + ["theme_flow_shock_reclaim_v2"], "max_positions": 5},
        {"name": "MVP_PLUS_THEME_SHOCK_SHARED_p6", "strategies": mvp_strategies + ["theme_flow_shock_reclaim_v2"], "max_positions": 6},
    ]

    _export_recent_passes(
        features=features,
        hypotheses=[hypotheses[name] for name in theme_strategies + theme_quality_strategies],
        start="2026-05-04",
        end=args.end,
        out_dir=out_dir,
    )
    ...
    for case in cases:
        ...
        print(f"[theme-flow] case={case['name']} strategies={len(case['strategies'])}")
        ...
        _export_strategy_breakdown(out_dir, stem)
    ...
    print(f"[theme-flow] saved {out_dir / 'summary.csv'}")
