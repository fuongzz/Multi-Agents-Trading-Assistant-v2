...
def _eligible_pool(day: pd.DataFrame, market_gate: str, pool_filter: str) -> pd.DataFrame:
    if market_gate == "strict":
        market_mask = (
...
def _select_symbols(
    ...
    pool = _eligible_pool(day, market_gate, pool_filter)
    if pool.empty:
        return []
    score_col = _score_column(score_mode)
    return list(pool.sort_values(score_col, ascending=False).head(positions)["symbol"])
    ...
    *,
    ...
    for symbol in list(positions):
        ...
        if excess_value > cfg.min_trade_value:
            shares_to_sell = int(excess_value / float(open_price))
...
def run_backtest(cfg: RotationConfig, features: pd.DataFrame | None = None) -> dict[str, Any]:
    if features is None:
    ...
    for idx, date in enumerate(calendar):
        day = by_date[date]
        open_by_symbol = day.set_index("symbol")["open"]
        todays_close = day.set_index("symbol")["close"].dropna()
        ...
        if idx % cfg.rebalance_days == 0:
            if cfg.selection_mode == "turnover_aware":
                ...
                target = _select_symbols(day, cfg.positions, cfg.market_gate, cfg.pool_filter, cfg.score_mode)
    ...
    if calendar and positions:
        ...
        final_close = by_date[final_date].set_index("symbol")["close"].dropna()
